...
> [Github Repository](https://github.com/marcabounader/SMTPwn)
...
SMTPwn is a Python-based SMTP user enumeration tool designed for penetration testers, bug bounty hunters, and security researchers. It abuses the SMTP protocol to discover valid usernames on a mail server — a classic recon technique that still works on a surprising number of real-world targets.

It supports three native SMTP enumeration methods (`VRFY`, `RCPT TO`, `EXPN`) and a `BOTH` mode that requires a user to pass both VRFY and RCPT checks, giving near-zero false positives on catch-all servers.

Useful for: OSCP exam prep, HackTheBox machines, TryHackMe labs, CTF challenges, and real-world email security assessments.

Tested against: Postfix, Sendmail, Microsoft Exchange, HMailServer.
...
| `--timeout` | Socket timeout (seconds) | `15.0` |
...
Let the tool extract the domain from the banner automatically:
```bash
python3 smtp_enum.py -t 10.10.10.10 -w users.txt -m VRFY
```
...
Skip pre-flight and go straight to scan:
```bash
python3 smtp_enum.py -t 10.10.10.10 -d target.com -w users.txt --no-preflight
```

Pre-flight on selected method only (faster):
```bash
...
`smtp-enumeration` `smtp-user-enum` `pentesting` `recon` `oscp` `ctf` `bugbounty` `email-security` `vrfy` `rcpt` `expn` `smtp-relay` `kali-linux` `hackthebox` `tryhackme` `python` `network-security`
